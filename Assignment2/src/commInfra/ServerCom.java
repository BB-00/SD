 ...
 *
 * Communication is based on message passing over sockets using the TCP
 * protocol. It supposes the setup of a communication channel between the two
 * end points before data transfer can take place. Data transfer is
 * bidirectional and is made through the transmission and the reception of
 * objects in output and input streams, respectively.
 */
public class ServerCom {
	...
	/**
	 * Number of the listening port at the computational system where the server is
	 * located.
	 */
	 ...
	 * Service setup.
	 *
	 * Instantiation of the listening socket, its binding to the public server
	 * address and port number and fixing a listening timeout.
	 */
	public void start() {
		try {
		...
		} catch (BindException e) // fatal error --- port already in use
		{
			GenericIO.writelnString(Thread.currentThread().getName()
					+ " - it was not possible the association of the listening socket to the port: " + serverPortNumb
					+ "!");
			e.printStackTrace();
			System.exit(1);
		} catch (SocketException e) {
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - an error has occurred on fixing a listening timeout!");
			e.printStackTrace();
			System.exit(1);
		} catch (IOException e) // fatal error --- other reasons
		{
			GenericIO.writelnString(Thread.currentThread().getName()
					+ " - an indeterminate error has occurred in establishing the connection at: " + serverPortNumb
					+ "!");
			e.printStackTrace();
			System.exit(1);
		}
	...
	public void end() {
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to close the listening socket!");
			e.printStackTrace();
			System.exit(1);
		}
	}
	 ...
	 * Listening process.
	 *
	 * Instantiation of a communication channel for a pending request. Instantiation
	...
	public ServerCom accept() throws SocketTimeoutException {
		...
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to open the input stream!");
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to open the output stream!");
			e.printStackTrace();
			System.exit(1);
		}
	 ...
	 *
	 * The socket input and output streams are closed. The communication socket is
	 * closed.
	 */
	public void close() {
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to close the input stream!");
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to close the output stream!");
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - it was not possible to close the communication socket!");
			e.printStackTrace();
			System.exit(1);
		}
	}
...
//		  GenericIO.writelnString();
			...
			GenericIO.writelnString(Thread.currentThread().getName() + " - the read object could not be deserialized!");
			e.printStackTrace();
			System.exit(1);
		} catch (IOException e) {
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - error on reading an object from the input stream!");
			e.printStackTrace();
			System.exit(1);
		} catch (ClassNotFoundException e) {
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - the read object belongs to an unknown data type!");
			e.printStackTrace();
			System.exit(1);
		}
...
//	   GenericIO.writelnString();
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - the object to be written can not be serialized!");
			e.printStackTrace();
			System.exit(1);
			...
			GenericIO.writelnString(
					Thread.currentThread().getName() + " - error on writing an object to the output stream!");
			e.printStackTrace();
			System.exit(1);
		}
	}
